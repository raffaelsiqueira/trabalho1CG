 ...
 */
bool AVL::add(int data, int line){
	...
	if (root == NULL){
		...
		if (contains(data, root) == NULL){
			root = recursiveAdd(data, line, root);
			was_added = true;
		} else {
			was_added = false;
		}
...
Node* AVL::recursiveAdd(int data, int line, Node* current_search){
	if (current_search == NULL){
		Node* location_added = new Node;
		location_added->data = data;
		location_added->line = line;
		return balance(location_added);
	}
	
	if (data > current_search->data){
		current_search->right_child = recursiveAdd(data, line, current_search->right_child);
	} else {
		current_search->left_child = recursiveAdd(data, line, current_search->left_child);
	}
	
	return balance(current_search);
}

int AVL::pred(int data, Node* current_search){
	if (current_search->data == data  && current_search->left_child != NULL){
		
		return pred(data, current_search->left_child);
	}
	else if (current_search->right_child != NULL){
		
		return pred(data, current_search->right_child);
	}
	else{
		
		return current_search->line;
	}
}

int AVL::succ(int data, Node* current_search){
	if (current_search->data == data  && current_search->right_child != NULL){
		return pred(data, current_search->right_child);
	}
	else if (current_search->left_child != NULL){
		return pred(data, current_search->left_child);
	}
	else{
		return current_search->line;
